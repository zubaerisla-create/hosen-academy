<form class="required-form" action="{{ route('instructor.live.class.store', ['course_id' => $course_id]) }}"
    ...
    <div class="fpb-7 mb-3">
        <label class="form-label ol-form-label" for="class_topic">{{ get_phrase('Class Topic') }}<span
    ...
    <div class="fpb-7 mb-3">
        <label class="form-label ol-form-label" for="class_date_and_time">{{ get_phrase('Class date and time') }}<span
                class="required">*</span></label>
        <input type="datetime-local" class="form-control ol-form-control" name="class_date_and_time"
            id="class_date_and_time" required>
    </div>
