...
@endphp
<form class="required-form" action="{{ route('instructor.live.class.update', ['id' => $id]) }}" method="post">
    @csrf
    ...
    <div class="fpb-7 mb-3">
        ...
        <select class="ol-select2" name="user_id" id="live_class_instructor_id">
            @foreach (App\Models\Course::where('id', $live_class->course_id)->first()->instructors() as $instructor)
                <option value="{{ $instructor->id }}" @if ($live_class->user_id == $instructor->id) selected @endif>
