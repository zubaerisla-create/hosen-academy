    ...
    <div class="col-12">
        <div id="lesson-list" class="list-group d-grid gap-2 border-0 mb-3">
            @foreach ($lessons as $key => $lesson)
                <div class="list-group-item rounded-3 py-2 px-2 border-1 draggable-item hover-parent d-flex"id="{{ $lesson->id }}">
...
<script>
    ...
    function sort(id) {
        ...
        $.ajax({
            url: "{{ route('instructor.lesson.sort') }}",
            type: 'POST',
            data: {
                itemJSON: itemJSON,
                course_id: course_id,
            ...
            success: function(response) {
                success("{{ get_phrase('Lesson sorted successfully.') }}");
