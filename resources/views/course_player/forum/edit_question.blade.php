...
<div class="">
    <form action="{{ route('forum.question.update', $question->id) }}" method="POST">
        @csrf
        <div class="search-box">
            <input type="hidden" name="course_id" value="{{ $course_id }}">
            <div class="heading">
                <h1>{{ get_phrase('Title or summary') }}</h1>
            </div>
            <input type="text" name="title" value="{{ $question->title }}" class="form-control" placeholder="Title">

            <div class="summernote">
                <h1>{{ get_phrase('Details') }}</h1>
                <textarea name="description" class="form-control" id="summernote" cols="30" rows="10">{!! removeScripts($question->description) !!}</textarea>
            </div>
