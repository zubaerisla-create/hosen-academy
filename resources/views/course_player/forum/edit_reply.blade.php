...
<div class="">
    <form action="{{ route('forum.question.update', $reply->id) }}" method="POST">
        @csrf
        <input type="hidden" name="title" value="reply">
        <div class="search-box">
            <div class="summernote">
                <h1>{{ get_phrase('Reply') }}</h1>
                <textarea name="description" class="form-control" id="summernote">{!! removeScripts($reply->description) !!}</textarea>
            </div>
