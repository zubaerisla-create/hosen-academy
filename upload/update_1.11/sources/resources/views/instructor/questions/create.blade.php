...
<form class="ajaxForm" action="{{ route('instructor.course.question.store') }}" method="post">@csrf
        ...
        <div class="col-sm-12">
            <div class="mb-3">
                ...
                <select class="form-control ol-form-control ol-select2" data-toggle="select2" name="type"
                    ...
                    <option value="single_choice">{{ get_phrase('Single Choice') }}</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex gap-3">
        <a href="#" class="btn ol-btn-primary" id="questionBackBtn"
            onclick="ajaxModal('{{ route('modal', ['instructor.questions.index', 'id' => $id]) }}', '{{ get_phrase('Questions') }}', 'modal-lg')">
