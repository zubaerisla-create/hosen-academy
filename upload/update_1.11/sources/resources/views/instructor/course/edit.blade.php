...
@section('content')
    ...
    <div class="col-md-12">
        <form action="{{ route('instructor.course.update', $course_details->id) }}" method="post" enctype="multipart/form-data">@csrf
            <div class="ol-card">
                    ...
                    <div class="d-flex gap-3 flex-wrap flex-md-nowrap">
                        <div class="ol-sidebar-tab">
                            <div class="d-flex flex-column">
                                ...
                                <a class="nav-link @if ($tab == 'assignment') active @endif" href="{{ route('instructor.course.edit', [$param, 'tab' => 'assignment']) }}">
                                    <span class="fi fi-rr-memo-pad"></span>
                                    <span>{{ get_phrase('Assignment') }}</span>
                                </a>

                                <a class="nav-link @if ($tab == 'course-progress') active @endif" href="{{ route('instructor.course.edit', [$param, 'tab' => 'course-progress']) }}">
                                    <span class="fi-rr-bars-progress"></span>
                                    <span>{{ get_phrase('Academic progress') }}</span>
                                </a>
                                ...
                                <a class="nav-link @if ($tab == 'custom-field') active @endif" href="{{ route('instructor.course.edit', [$param, 'tab' => 'custom-field']) }}">
                                    <span class="fi-rr-layer-plus"></span>
                                    <span>{{ get_phrase('Custom Field') }}</span>
                                </a>
                                <a class="nav-link @if ($tab == 'notice_board') active @endif" href="{{ route('instructor.course.edit', [$param, 'tab' => 'notice_board']) }}">
                        ...
                        <div class="tab-content w-100">
                            ...
                            @includeWhen($tab == 'assignment', 'instructor.course.assignment')
                             @includeWhen($tab == 'course-progress', 'instructor.course.academic_progress')
                            ...
                            @includeWhen($tab == 'custom-field', 'instructor.course.edit_custom_field')
                            @includeWhen($tab == 'notice_board', 'instructor.course.notice.notice_board')
                        </div>
                    </div>
                </div>
            </div>
