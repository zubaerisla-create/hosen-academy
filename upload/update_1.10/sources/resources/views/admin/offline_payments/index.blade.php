                                                    ...
                                                    <div class="dAdmin_profile_name">
                                                        @if ($payment->item_type == 'course')
                                                            @foreach (App\Models\Course::whereIn('id', json_decode($payment->items, true))->get() as $course)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('course.details', slugify($course->title)) }}" target="_blank" class="text-muted me-3">{{ $course->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif
                                                        ...
                                                        @if ($payment->item_type == 'bootcamp')
                                                            @foreach (App\Models\Bootcamp::whereIn('id', json_decode($payment->items, true))->get() as $bootcamp)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('bootcamp.details', ['slug' => slugify($bootcamp->title)]) }}" class="text-muted me-3" target="_blank">{{ $bootcamp->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif
                                                        @if ($payment->item_type == 'ebook')
                                                            @foreach (App\Models\Ebook::whereIn('id', json_decode($payment->items, true))->get() as $ebook)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('ebook.details', ['slug' => slugify($ebook->title)]) }}" class="text-muted me-3" target="_blank">{{ $ebook->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif
                                                        ...
                                                        @if ($payment->item_type == 'package')
                                                            @foreach (App\Models\TeamTrainingPackage::whereIn('id', json_decode($payment->items, true))->get() as $package)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('team.package.details', ['slug' => $package->slug]) }}" target="_blank" class="text-muted me-3">{{ $package->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif
                                                        ...
                                                        @if ($payment->item_type == 'tutor_booking')
                                                            ...
                                                            @endforeach
                                                        @endif
                                                         @if ($payment->item_type == 'bundle')
                                                            @foreach (App\Models\CourseBundle::whereIn('id', json_decode($payment->items, true))->get() as $bundle)
                                                                <p class="sub-title text-12px">
                                                                    <a href="{{ route('course.bundle.details', slugify($bundle->title)) }}" target="_blank" class="text-muted me-3">{{ $bundle->title }} </a>
                                                                </p>
                                                            @endforeach
                                                        @endif
                                                    </div>
