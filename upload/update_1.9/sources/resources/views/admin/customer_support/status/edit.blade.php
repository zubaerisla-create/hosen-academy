<form action="{{ route('admin.customer.support.ticket.status.update', ['id' => $status->id]) }}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="fpb7 mb-2">
        <label class="form-label ol-form-label" for="title">{{ get_phrase('Title') }}</label>
        <input class="form-control ol-form-control" type="text" id="title" name="title" value="{{ $status->title }}" required>
    </div>
    ...
    <div class="fpb-7 mb-3">
        ...
        <select class="form-control ol-form-control ol-select2" name="status" id="status" required>
            <option value="1" {{ $status->status == 1 ? 'selected' : '' }}>{{ get_phrase('Active') }}</option>
            <option value="0" {{ $status->status == 0 ? 'selected' : '' }}>{{ get_phrase('De Active') }}</option>
        </select>
    </div>
    ...
    <div class="form-check">
        <input type="checkbox" id="view-10" name="default_view" class="form-check-input" value="1" {{ $status->default_view ? 'checked' : '' }}>
    ...
    <div class="fpb7 mb-3">
        ...
        <input type="color" class="form-control form-control-color" name="color" id="color" value="{{ $status->color ?? '#007bff' }}">
    </div>
