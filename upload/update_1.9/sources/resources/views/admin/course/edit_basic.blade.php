...
<div class="row mb-3">
    <label for="title" class="form-label ol-form-label col-sm-2 col-form-label">{{ get_phrase('Instructor of this course') }}</label>
    <div class="col-sm-10">
        <div id="instructor_list" class="d-flex flex-column gap-2">

            @php
                $instructor_ids = json_decode($course_details->instructor_ids, true);
            @endphp

            @if (empty($instructor_ids) || $instructor_ids === [null])
                @php
                    $instructor = App\Models\User::find($course_details->user_id);
                @endphp
                @if ($instructor)
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <div>
                            <img class="img-fluid rounded-circle image-30" width="30" height="30" src="{{ get_image($instructor->photo) }}">
                        </div>
                        <div class="ms-1 align-items-center d-flex">
                            <h4 class="title fs-14px mb-0">{{ $instructor->name }}</h4>

                            <a class="btn text-danger fs-12px ms-2" onclick="ajaxModal('{{ route('admin.course.change_course_author', $course_details->id) }}' ,'{{ get_phrase('Change Author') }}' )" href="javascript:void(0)">{{ get_phrase('Change Course Author') }}</a>
                        </div>
                    </div>
                @endif
            @else
                @php
                    $instructors = App\Models\User::whereIn('id', $instructor_ids)->get();
                @endphp
                ...
                @foreach ($instructors as $instructor)
                    <div class="d-flex align-items-center gap-2 mb-2">
                        ...
                        <div class="ms-1 align-items-center d-flex">
                            ...
                            @if ($course_details->user_id == $instructor->id)
                                @if (count($instructors) > 1)
                                    <a class="btn text-danger fs-12px ms-2" onclick="ajaxModal('{{ route('admin.course.change_course_author', $course_details->id) }}' ,'{{ get_phrase('Change Author') }}' )" href="javascript:void(0)">{{ get_phrase('Change Course Author') }}</a>
                                @endif
                            @else
                                <a class="btn text-danger fs-12px ms-2" onclick="confirmModal('{{ route('admin.course.remove_an_instructor', [$course_details->id, $instructor->id]) }}')" href="javascript:void(0)">{{ get_phrase('Remove') }}</a>
                            @endif
                        </div>
                    </div>
                @endforeach
...
<div class="row mb-3">
    <label for="instructor_id" class="form-label ol-form-label col-sm-2 col-form-label">{{ get_phrase('Add new instructor') }}</label>
    <div class="col-sm-10">
        <select class="ol-select2" name="instructors[]" id="instructor_id" multiple="multiple" data-placeholder="Choose ...">
            @php
                $all_instructors = App\Models\User::whereIn('role', ['admin', 'instructor'])
                    ->where('status', 1)
                    ->get();
            @endphp
            @foreach ($all_instructors as $all_instructor)
                <option value="{{ $all_instructor->id }}">{{ $all_instructor->name }} ( {{ $all_instructor->email }} ) </option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="row mb-3 ">
    <label for="course_status" class="form-label ol-form-label col-sm-2 col-form-label">{{ get_phrase('Create as') }} <span class="text-danger ms-1">*</span></label>
    <div class="col-sm-10">
        <div class="eRadios">
            ...
            <div class="form-check">
                <input type="radio" value="inactive" name="status" class="form-check-input eRadioDark" id="status_inactive" @if ($course_details->status == 'inactive') checked @endif required>
