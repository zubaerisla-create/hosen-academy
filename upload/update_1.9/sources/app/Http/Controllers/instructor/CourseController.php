    ...
    public function index(Request $request)
    {
        ...
        $query = Course::where(function ($q) {
            $q->where('user_id', auth()->id())
                ->orWhereJsonContains('instructor_ids', (string)auth()->id());
        });
        ...
        $page_data['pending_courses'] = Course::where('status', 'pending')
            ->where(function ($q) {
                $q->where('user_id', auth()->id())
                    ->orWhereJsonContains('instructor_ids', (string)auth()->id());
            })->count();
        $page_data['active_courses'] = Course::where('status', 'active')
            ->where(function ($q) {
                $q->where('user_id', auth()->id())
                    ->orWhereJsonContains('instructor_ids', (string)auth()->id());
            })->count();

        $page_data['upcoming_courses'] = Course::where('status', 'upcoming')
            ->where(function ($q) {
                $q->where('user_id', auth()->id())
                    ->orWhereJsonContains('instructor_ids', (string)auth()->id());
            })->count();

        $page_data['paid_courses'] = Course::where('is_paid', 1)
            ->where(function ($q) {
                $q->where('user_id', auth()->id())
                    ->orWhereJsonContains('instructor_ids', (string)auth()->id());
            })->count();

        $page_data['free_courses'] = Course::where('is_paid', 0)
            ->where(function ($q) {
                $q->where('user_id', auth()->id())
                    ->orWhereJsonContains('instructor_ids', (string)auth()->id());
            })->count();
