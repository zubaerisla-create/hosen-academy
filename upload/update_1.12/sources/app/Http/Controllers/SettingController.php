...
class SettingController extends Controller
{
    ...
    // public function zapier_settings($active_tab = "")
    // {
    //     $page_data = array();
    ...
    // function zapier_settings_update(Request $request, $slug = "")
    // {
    //     if (!empty($request->all())) {
    //         $updateZap = ZapierSetting::where('slug', $slug)->first();
    //         $updateZap->webhook_url = $request->webhook_url;
    //         $updateZap->status = $request->status;

    //         $updateZap->save();
    //         $page_data = array();
    //         $page_data['zaps'] = ZapierSetting::all();
    //         $page_data['active_tab'] = $slug;

    //         return redirect('/admin/zapier_settings/' . $slug)->with('success', 'Zaps updated Successfully');
    //     }
    ...
    {
        return view('admin.setting.google_setting');
    }

    public function google_settings_update(Request $request)
    {
        $fields = [
            'google_login_status',
            'google_client_id',
        ...
        foreach ($fields as $field) {
            ...
            if (Setting::where('type', $field)->count()) {
                Setting::where('type', $field)->update(['description' => $value]);
            } else {
                Setting::insert(['type' => $field, 'description' => $value]);
            }
        }

        Session::flash('success', get_phrase('Google settings updated successfully'));
        return redirect()->back();
    }

    // Gamification Badges Settings
    public function gamification_settings()
    {
        $page_data['badges'] = Badge::all();
        return view('admin.gamification_badge.index', $page_data);
    }

    public function gamification_badges_store(Request $request)
    {
        $request->validate([
            'type'           => 'required|string',
            'title'          => 'required|string|max:255',
            'condition_from' => 'required|integer|min:0',
            'condition_to'   => 'required|integer|gte:condition_from',
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('uploads/badges'), $imageName);
        }
        ...
        Badge::create([
            'type'           => $request->type,
            'title'          => $request->title,
            'condition_from' => $request->condition_from,
            'condition_to'   => $request->condition_to,
            'description'    => $request->description,
            'image'          => $imageName ?? null,
        ]);

        Session::flash('success', get_phrase('Gamification badge created successfully'));

        return redirect()->back();
    }

    public function gamification_badges_update(Request $request, $id)
    {
        // $request->validate([
        //     'type'           => 'required|string',
        //     'title'          => 'required|string|max:255',
        ...
        if ($request->hasFile('image')) {
            ...
            if ($badge->image && file_exists(public_path('uploads/badges/' . $badge->image))) {
                unlink(public_path('uploads/badges/' . $badge->image));
            }
        ...
        $badge->condition_from = $request->condition_from;
        $badge->condition_to   = $request->condition_to;
        $badge->description    = $request->description;
        $badge->save();

        Session::flash('success', get_phrase('Gamification badge updated successfully'));

        return redirect()->back();
    }

    public function gamification_badges_delete($id)
    {
        $badge = Badge::findOrFail($id);
        if ($badge->image && file_exists(public_path('uploads/badges/' . $badge->image))) {
            unlink(public_path('uploads/badges/' . $badge->image));
        ...
        $badge->delete();
        Session::flash('success', get_phrase('Gamification badge deleted successfully'));
        return redirect()->back();
    }
